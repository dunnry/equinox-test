module DirectIngestor.Ingester
...
type Stats(log, statsInterval, stateInterval) =
    ...
    override __.HandleOk res =
        res
        |> function
        | Outcome.Ok (used, unused) ->
            ok <- ok + used
            skipped <- skipped + unused
...
let handle (sourceService: Todo.Service)
           (summaryService: TodoSummary.Service)
           (stream, span: Propulsion.Streams.StreamSpan<_>)
           =
    async {
        ...
        | Todo.Events.Match (clientId, events) when events |> Seq.exists Todo.Fold.impliesStateChange ->
            ...
            | true ->
                return Propulsion.Streams.SpanResult.OverrideWritePosition version',
                       Outcome.Ok(1, span.events.Length - 1)
            | false ->
                return Propulsion.Streams.SpanResult.OverrideWritePosition version', Outcome.Skipped span.events.Length
